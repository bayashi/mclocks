...
pub fn handle_editor_request(
    ...
    match request.method() {
        ...
        Method::Post => handle_post_request(
            request,
            repos_dir,
            include_host,
            editor_command,
...
fn handle_post_request(
    ...
    if let Err(_) = request
        .as_reader()
        .take(MAX_EDITOR_BODY_BYTES as u64 + 1)
        .read_to_end(&mut body)
    {
        return create_error_html_response_with_status(
            StatusCode(400),
    ...
    let owner_repo_for_link = parsed_for_link
        .as_ref()
    ...
    if token.is_empty() {
        return create_error_html_response_with_status(
            StatusCode(403),
            "Forbidden: Missing token. Reload and try again.",
            owner_repo_for_link.as_deref(),
            href_for_link.as_deref(),
            None,
            None,
        );
    }
    ...
    if !consume_one_time_token(token, &normalized_path) {
        return create_error_html_response_with_status(
            StatusCode(403),
            "Forbidden: Invalid or expired token. Reload and try again.",
            owner_repo_for_link.as_deref(),
            href_for_link.as_deref(),
            None,
            None,
    ...
    let local_path = match convert_github_path_to_local(
        &normalized_path,
        repos_dir,
        &params.repos_dir,
        include_host,
    ...
    if let Some(ch) = find_unsafe_path_char(&file_for_check) {
        return create_error_html_response_with_status(
            StatusCode(400),
            &format!("Unsafe character '{}' in local file path", ch),
            owner_repo_for_link.as_deref(),
            href_for_link.as_deref(),
            Some(file_for_check),
            Some(command_preview),
    ...
    match execute_command(editor_command, &rendered_args) {
        ...
        Err(e) => create_error_html_response_with_status(
            StatusCode(500),
            &e,
            owner_repo_for_link.as_deref(),
            href_for_link.as_deref(),
...
fn create_error_html_response_with_status(
    status_code: StatusCode,
    message: &str,
    owner_repo: Option<&str>,
    owner_repo_href: Option<&str>,
    local_full_path: Option<String>,
...
fn parse_repo_path_best_effort(path: &str) -> Option<ParsedRepoPath> {
    ...
    if segments.len() >= 6 {
        ...
        if blob_or_tree == "blob" || blob_or_tree == "tree" {
            ...
            let file_path_parts: Vec<String> =
                segments[5..].iter().map(|s| s.to_string()).collect();
...
fn normalize_repos_dir(repos_dir: &str) -> Result<String, String> {
    ...
    if !path.exists() {
        return Err(format!(
            "web.editor.reposDir does not exist: {}",
            normalized
        ));
    }
    if !path.is_dir() {
...
fn check_command_exists(command: &str) -> Result<(), String> {
    ...
    match Command::new(lookup).arg(command).output() {
...
mod tests {
    ...
    fn test_token_one_time_and_path_bound() {
        with_test_lock(|| {
            ...
            assert!(!consume_one_time_token(
                &token2,
                "/o/r/blob/main/src/other.rs"
            ));
        });
    }
    ...
    fn test_get_local_lib_path_owner_repo_without_host() {
        with_test_lock(|| {
            ...
            let expected = std::path::PathBuf::from("o")
                .join("r")
                .join("src")
                .join("lib.rs");
    ...
    fn test_get_local_lib_path_with_host_when_enabled() {
        with_test_lock(|| {
            ...
            let expected = std::path::PathBuf::from("github.com")
                .join("o")
                .join("r")
                .join("src")
                .join("lib.rs");
    ...
    fn test_find_unsafe_path_char() {
        with_test_lock(|| {
            ...
            assert_eq!(
                find_unsafe_path_char("C:\\bad\\%COMSPEC%\\file.txt"),
                Some('%')
            );
        });
    }
    ...
    fn test_check_command_exists_nonexistent_command() {
        with_test_lock(|| {
            ...
            assert!(
                err.contains("Command not found in PATH"),
                "unexpected error: {}",
                err
            );
        });

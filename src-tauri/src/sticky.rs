...
pub fn create_sticky(
    app: AppHandle,
    cfg_state: State<'_, Arc<ContextConfig>>,
    sticky_store: State<'_, StickyInitStore>,
    persist: State<'_, StickyPersistStore>,
    text: String,
    ...
    {
        let mut map = sticky_store
            .init_by_window_label
            .lock()
            .map_err(|_| "Failed to lock sticky store".to_string())?;
        map.insert(
            label.clone(),
            StickyInitContent {
                text: text.clone(),
    ...
    // Persist to file
    {
        let mut data = persist
            .data
            .lock()
            .map_err(|_| "Failed to lock persist store".to_string())?;
...
pub fn create_sticky_image(
    ...
    if image_bytes.len() > MAX_IMAGE_BYTES {
        ...
        return Err(format!(
            "Image is too large ({:.1} MB). Maximum size is 10 MB.",
            size_mb
        ));
    }
...
#[tauri::command]
pub fn sticky_take_init_content(
    sticky_store: State<'_, StickyInitStore>,
    id: String,
) -> Result<StickyInitContent, String> {
    let mut map = sticky_store
        .init_by_window_label
        .lock()
        .map_err(|_| "Failed to lock sticky store".to_string())?;
    Ok(map.remove(&id).unwrap_or_else(|| StickyInitContent {
        text: String::new(),
        content_type: None,
    }))
}
...
pub fn load_sticky_image(
    ...
    let filename = sticky
        .image_filename
        .as_ref()
        .ok_or("No image for this sticky")?;
...
pub fn save_sticky_text(
    ...
    let entry = data
        .entry(id)
        .or_insert_with(|| StickyData::new(String::new()));
    entry.text = text;
...
#[tauri::command]
pub fn save_sticky_state(
    persist: State<'_, StickyPersistStore>,
    id: String,
    is_open: bool,
    open_width: Option<f64>,
...
#[tauri::command]
pub fn load_sticky_state(
    persist: State<'_, StickyPersistStore>,
    id: String,
) -> Result<Option<StickyStateInfo>, String> {
    let data = persist
        .data
        .lock()
        .map_err(|_| "Failed to lock persist store".to_string())?;
...
pub fn restore_stickies(
    ...
    let notes = {
        ...
        let known_images: std::collections::HashSet<String> = data
            .values()

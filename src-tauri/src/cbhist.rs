...
use crate::config::{ContextConfig, save_clipboard_window_dimensions};
...
fn maybe_record_clipboard_update<R: Runtime>(handle: &AppHandle<R>, store: &CbhistStore) {
    ...
    if dq.front().map(|e| e.text.as_str()) == Some(normalized.as_str()) {
        return;
    }
...
pub fn show_cbhist_panel<R: Runtime>(app: &AppHandle<R>) {
    ...
    let (lw, lh) = store.panel_size().unwrap_or((420.0, 480.0));
    let tray_rect = app.tray_by_id("main").and_then(|t| t.rect().ok().flatten());
...
) -> Result<Vec<CbhistItemDto>, String> {
    if store.disabled {
        return Ok(Vec::new());
    }
    let dq = store

...
**Markdown**은 **변경을 자동으로 감지하고 브라우저에 실시간으로 반영**됩니다(**`source`** 모드의 렌더링 표시).
...
**`mclocks-datetime-util`** [MCP](https://modelcontextprotocol.io/) 서버는 [Cursor](https://www.cursor.com/), [Claude Desktop](https://claude.ai/download) 같은 어시스턴트가 여러 시간대에서 날짜/시간 질문에 답할 수 있게 합니다. mclocks `config.json`을 읽을 수 있습니다.

설정·도구·개발: **[bayashi/mclocks-datetime-util](https://github.com/bayashi/mclocks-datetime-util)** ([npm](https://www.npmjs.com/package/mclocks-datetime-util) 도 있음).

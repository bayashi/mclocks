...
Générateur en ligne de `config.json` : https://bayashi.github.io/mclocks/mclocks-config-generator/

### Exemple de config.json pour l'horloge

Le fichier `config.json` doit être formaté en JSON (compatible JSONC), comme indiqué ci-dessous.
...
Les sections ci-dessous décrivent les champs que vous pouvez définir dans `config.json`.
...
Ajoutez ce qui suit à la configuration `web` pour `editor` :
...
Le serveur [MCP](https://modelcontextprotocol.io/) **`mclocks-datetime-util`** permet aux assistants tels que [Cursor](https://www.cursor.com/) et [Claude Desktop](https://claude.ai/download) de répondre aux questions de date/heure multi-fuseaux, et plus. Il peut lire votre `config.json` mclocks.

Configuration, outils et développement : **[bayashi/mclocks-datetime-util](https://github.com/bayashi/mclocks-datetime-util)** (aussi sur [npm](https://www.npmjs.com/package/mclocks-datetime-util)).

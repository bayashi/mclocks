...
ブラウザ上の `config.json` 生成ツール: https://bayashi.github.io/mclocks/mclocks-config-generator/

### 時計用の config.json の例

`config.json` ファイルは以下のように JSON 形式（JSONC 互換）で記述します。
...
以下の各節で、`config.json` に設定できるフィールドを説明します。
...
ショートカットの一覧表（ヘルプ表示、設定・表示フォーマット、タイマー、付箋メモ、クリップボードの日時操作、テキスト変換）は **[`mclocks-cheat-sheet.md`](../mclocks-cheat-sheet.md)** で管理しています。
...
`mclocks` は起動時に常に内蔵のローカル Web サーバーを立ち上げます。`config.json` に `web` フィールドを設定すると、さらに指定ディレクトリから静的ファイルを配信できます:
...
* `root`: 配信するファイルのあるディレクトリへのパス（静的ファイルホスティングを使う場合のみ必須）
* `port`: メイン Web サーバーの希望ポート番号（`>=2000`、デフォルト: `3030`）。希望ポートが使用中の場合、mclocks は利用可能なポートが見つかるまで番号を下げて（`-1`）探します。
* `openBrowserAtStart`: `true` にすると、`mclocks` 起動時にデフォルトブラウザで Web サーバーの URL を自動的に開きます（デフォルト: `false`）
* `dump`: `true` にすると、リクエスト詳細を JSON で返す `/dump` エンドポイントを有効にします（デフォルト: `false`）
* `slow`: `true` にすると、応答を遅延させる `/slow` エンドポイントを有効にします（デフォルト: `false`）
...
静的ファイル配信に加え、mclocks はドラッグ＆ドロップのコンテンツビューアーに対応しています。

* ディレクトリを時計ウィンドウにドロップすると、一時的なローカル URL で Web ビューアーで開きます。
* 単一ファイルをドロップした場合、一時ファイルビューアーが対応している形式であれば Web ビューアーで開きます。
* 生成された一時 URL はローカル専用で、mclocks 終了時に破棄されます。

#### コンテンツモード

Web ビューアーは `content`、`raw`、`source` などの `mode` クエリに対応しています。

* `content`（デフォルト）: 検出されたコンテンツタイプで配信し、可能な限りブラウザが通常どおり表示します。
* `raw`: 非バイナリを `text/plain` として返し、ブラウザ側レンダリングを避けて生テキストとして安全に表示します。
* `source`: 対応形式では要約／サイドバー付きのソースビューレイアウトを開き、非対応のテキストファイルでも安全にプレーンテキスト確認ができます。

**Markdown** は**変更を自動で検知し、リアルタイムに反映**されます（**`source`** モードのレンダリング表示）。
...
`mclocks-datetime-util` [MCP](https://modelcontextprotocol.io/) サーバーは、[Cursor](https://www.cursor.com/) や [Claude Desktop](https://claude.ai/download) などの AI アシスタントが複数タイムゾーンで日時に答えたり、日時とエポックを変換したりできます。mclocks の `config.json` を読み、設定した時計が AI の応答に反映されます。

セットアップ・ツール・開発は別リポジトリです: **[bayashi/mclocks-datetime-util](https://github.com/bayashi/mclocks-datetime-util)**（[npm](https://www.npmjs.com/package/mclocks-datetime-util) もあります）。

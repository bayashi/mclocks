...
* `openBrowserAtStart`: `true` に設定すると、`mclocks` 起動時にデフォルトブラウザで Web サーバーの URL を自動的に開きます（デフォルト: `false`）
...
Web ビューアーは `content` / `raw` / `source` の `mode` クエリをサポートしています。

* `content`（デフォルト）: 判定された Content-Type で配信し、可能な場合はブラウザで通常レンダリングされます。
* `raw`: 非バイナリファイルを `text/plain` として返し、ブラウザ側の意図しないレンダリングを避けて生テキストを表示します。
* `source`: サポートされる形式ではサマリー/サイドバー付きのソースビューを開き、未対応のテキストでも安全に内容を確認できます。

例: `http://127.0.0.1:3030/path/to/file.json?mode=raw`

### ファイル配信の挙動

Web サーバーは固定の拡張子 allowlist には限定されません。

リクエストされた各ファイルに対して、次のポリシーで配信します:

* Content-Type を判定できる場合は、その Content-Type で配信します。
* `mode=raw` では、非バイナリファイルを `text/plain` として配信します。
* Content-Type を判定できない場合、テキストファイルは `text/plain` で配信します。
* ブラウザで直接表示に向かないバイナリはダウンロードとして配信します。

### ドラッグ＆ドロップベースのコンテンツビューアー

静的ファイル配信に加えて、mclocks にはドラッグ＆ドロップベースのコンテンツビューアーがあります。

* ディレクトリを時計ウィンドウにドロップすると、一時的なローカルURLで Web ビューアーに開きます。
* 単体ファイルをドロップした場合は、一時ファイルビューアーで対応している形式のみ Web ビューアーで開きます。
* 生成される一時URLはローカル限定で、mclocks の終了時に破棄されます。

...
在线 `config.json` 生成器：https://bayashi.github.io/mclocks/mclocks-config-generator/
...
**Markdown** 会**自动检测更改并实时更新到浏览器**（**`source`** 模式下的渲染显示）。
...
**`mclocks-datetime-util`** [MCP](https://modelcontextprotocol.io/) 服务器让 [Cursor](https://www.cursor.com/)、[Claude Desktop](https://claude.ai/download) 等助手能回答跨时区的日期/时间问题，并进行更多操作。可读取得 mclocks `config.json`。

设置、工具与开发：**[bayashi/mclocks-datetime-util](https://github.com/bayashi/mclocks-datetime-util)**（也在 [npm](https://www.npmjs.com/package/mclocks-datetime-util)）。
